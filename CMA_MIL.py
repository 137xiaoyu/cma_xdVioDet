...
def CMAL(labels, logits, seq_len, audio_rep, visual_rep):
    ...
    for i in range(logits.shape[0]):
        if labels[i] > 0.5:
            cur_visual_inverse_topk, cur_visual_inverse_topk_indices = torch.topk(logits[i][:seq_len[i]], k=int(torch.div(seq_len[i], 16, rounding_mode='trunc') + 1), largest=False)
            ...
            cur_audio_inverse_topk, cur_audio_inverse_topk_indices = torch.topk(logits[i][:seq_len[i]], k=int(torch.div(seq_len[i], 16, rounding_mode='trunc') + 1), largest=False)
            ...
            cur_audio_topk, cur_audio_topk_indices = torch.topk(logits[i][:seq_len[i]], k=int(torch.div(seq_len[i], 16, rounding_mode='trunc') + 1), largest=True)
            ...
            cur_visual_topk, cur_visual_topk_indices = torch.topk(logits[i][:seq_len[i]], k=int(torch.div(seq_len[i], 16, rounding_mode='trunc') + 1), largest=True)
